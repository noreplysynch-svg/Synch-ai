...
now handled by the Express server in `/server`, backed by Postgres. AI calls
also moved server-side (Groq, free tier) — the browser no longer talks to any AI
provider directly.
...
3. **Set environment variables** on the service (Railway → Variables). See
   ...
   - `TAVILY_API_KEY` — free key from https://tavily.com (powers the web-search toggle)
...
| AI chat + Whisper (Groq, free tier) | Called directly from the browser (API key exposed client-side) | Proxied through `/api/chat/*` — key stays server-side only |
| Web search (Tavily, free tier) | N/A | Triggered per-message when the globe toggle in the chat box is on |
