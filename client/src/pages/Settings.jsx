...
function SecurityPanel() {
  ...
  const [setupSecret, setSetupSecret] = useState(null); // { secret, otpauthUrl } while enrolling
  ...
  const confirmEnroll = async () => {
    if (enrollCode.length !== 6) { toast.error('Enter the 6-digit code'); return; }
    setBusy(true);
    try {
      await auth.enable2FA(enrollCode);
      setEnabled(true);
      setSetupSecret(null);
      setEnrollCode('');
  ...
  const cancelEnroll = () => { setSetupSecret(null); setEnrollCode(''); };
  ...
  return (
    <div>
      ...
      <div className="py-4 border-b border-border">
        ...
        <Row label="Authenticator app" description="Require a 6-digit code from an authenticator app when signing in with your password.">
          <Switch
            checked={enabled}
            disabled={loadingStatus || busy}
            onCheckedChange={(v) => v ? startEnroll() : setShowDisableConfirm(true)}
          />
        </Row>

        {setupSecret && (
          <div className="mt-3 p-4 rounded-xl border border-border bg-secondary/20 space-y-3">
            <p className="text-sm font-medium">Scan or enter this key</p>
            <p className="text-xs text-muted-foreground">
              Add it to Google Authenticator, Authy, or 1Password, then enter the 6-digit code it generates.
            </p>
            <code className="block text-sm tracking-wider bg-background border border-border rounded-lg px-3 py-2">
              {setupSecret.secret}
            </code>
            <div className="flex items-center gap-2">
              <Input
                ...
                className="h-9 rounded-lg font-mono tracking-widest w-32"
              />
              <Button size="sm" onClick={confirmEnroll} disabled={busy}>Confirm</Button>
        ...
        {showDisableConfirm && (
          <div className="mt-3 p-4 rounded-xl border border-border bg-secondary/20 space-y-3">
            <p className="text-sm font-medium">Turn off the authenticator app?</p>
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <Button size="sm" variant="destructive" onClick={confirmDisable} disabled={busy}>Turn off</Button>
              <Button size="sm" variant="ghost" onClick={() => setShowDisableConfirm(false)}>Cancel</Button>
            </div>
          </div>
        )}
      </div>
...
export default function Settings({ onClose }) {
  ...
  return (
    <div
      className="fixed inset-0 flex items-center justify-center z-50 md:px-4"
    ...
    >
      <motion.div
        ...
        className="w-full h-full md:h-auto md:max-w-3xl bg-card md:border md:border-border md:rounded-2xl md:shadow-2xl overflow-hidden flex flex-col md:flex-row"
        style={{ maxHeight: '100vh' }}
      >
        {/* Nav — full screen on mobile until a section is opened, always a sidebar on desktop */}
        <div className={`w-full md:w-52 flex-shrink-0 md:border-r md:border-border bg-card flex-col ${mobilePanelOpen ? 'hidden md:flex' : 'flex'}`}>
          <div className="p-3 border-b border-border flex items-center gap-2">
            <button onClick={handleClose} className="w-9 h-9 rounded-lg flex items-center justify-center hover:bg-secondary transition-colors">
              <X className="w-4 h-4" />
            </button>
            ...
            {NAV_ITEMS.map(item => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-2.5 px-3 py-2.5 md:py-2 rounded-lg text-sm transition-colors ${
        ...
        {/* Panel — full screen on mobile once a section is opened, always visible on desktop */}
        <div className={`w-full flex-1 flex-col overflow-hidden ${mobilePanelOpen ? 'flex' : 'hidden md:flex'}`}>
          <div className="md:hidden p-3 border-b border-border flex items-center gap-2 flex-shrink-0">
            <button onClick={() => setMobilePanelOpen(false)} className="w-9 h-9 rounded-lg flex items-center justify-center hover:bg-secondary transition-colors">
              <ArrowLeft className="w-4 h-4" />
            </button>
            <span className="text-sm font-semibold">{NAV_ITEMS.find(i => i.id === activeSection)?.label}</span>
          </div>
