...
// turned on. It proves "we checked the password" without starting a real
// session — a session only begins once the 6-digit code also checks out.
export function signTwoFactorPendingToken(user) {
  return jwt.sign({ type: 'two_factor_pending', sub: user.id }, JWT_SECRET, { expiresIn: '5m' });
}

export function verifyTwoFactorPendingToken(token) {
  const payload = jwt.verify(token, JWT_SECRET);
  if (payload.type !== 'two_factor_pending') throw new Error('Invalid token type');
  return payload;
}
