...
export const providers = {
  ...
  microsoft: {
    // 'consumers' (not 'common') restricts sign-in to personal Microsoft
    // accounts (outlook.com, hotmail.com, live.com) — work/school (Azure AD
    // tenant) accounts are rejected by Microsoft itself before any consent
    // screen is shown.
...
export async function exchangeCodeForUser(providerName, code) {
  ...
  // Google Workspace (work/school) accounts carry an 'hd' claim; personal
  // Gmail accounts never do. Microsoft's work/school accounts are already
  // excluded at the /consumers/ endpoint above.
  if (providerName === 'google' && profile.hd) {
    const err = new Error('Please use a personal Google account, not a work or school account.');
    err.code = 'workspace_account_not_supported';
    throw err;
  }
